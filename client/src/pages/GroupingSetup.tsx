      ...
      <main className="flex-1 overflow-auto pb-20">
        <div className="max-w-4xl mx-auto p-4 space-y-6">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">Create player groupings for this round to enable group scoring mode.</p>
          </div>
          ...
          {selectedRoundId && (
            <>
                  ...
                  {unassignedPlayers.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                      {unassignedPlayers.map(player => (
                        <div
                          key={player.id}
                          className="flex items-center justify-between p-3 rounded border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition"
                        >
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-slate-600 dark:text-slate-400">{player.team?.name}</div>
                            </div>
                          </div>
                          <div className="text-xs bg-white dark:bg-slate-700 px-2 py-1 rounded">
                            HCP: {player.handicap}
                          </div>
                        </div>
                      ))}
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <div className="text-center py-8">
                        <p className="text-slate-500 dark:text-slate-400 mb-4">No groups created yet</p>
                      ...
                      <CardHeader className="flex flex-row items-center justify-between space-y-0">
                        <div className="flex-1">
                          ...
                          <input
                            ...
                            className="mt-2 w-full px-2 py-1 border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded text-sm"
                          />
                        </div>
                        <Button
                          variant="ghost"
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {group.playerIds.length > 0 && (
                          <div className="space-y-2">
                            <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Players in this group:</p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                              {group.playerIds.map(playerId => (
                                <div
                                  key={playerId}
                                  className="flex items-center justify-between p-3 rounded bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800"
                                >
                        ...
                        <div className="space-y-2">
                          <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Add player:</p>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {unassignedPlayers.map(player => (
                              <button
                                key={player.id}
                                onClick={() => addPlayerToGroup(group.groupNumber, player.id)}
                                className="flex items-center gap-3 p-3 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition text-left"
                              >
                                <div
                                  className="w-3 h-3 rounded-full"
                                  style={{ backgroundColor: getPlayerTeamColor(player.id) }}
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="text-xs text-slate-500 dark:text-slate-400">{player.team?.name}</div>
                                </div>
                              </button>
                            ))}
                          </div>
                          {unassignedPlayers.length === 0 && (
                            <div className="text-sm text-slate-500 dark:text-slate-400 py-2">No unassigned players</div>
                          )}
                        </div>
                      </CardContent>
                    </Card>
              ...
              {/* Action Buttons */}
              <div className="flex gap-3 sticky bottom-20 bg-white dark:bg-slate-800 dark:border-slate-700 p-4 rounded border">
